...
def dungeon(sio, data, btn_name, character_name):
  ...
  if btn_name=="격전의섬":
    ...
    keyboard('v')
    client_utils.searchImg('stop_schedule.png',beforeDelay=0, afterDelay=0, _region=(1260,790,300,100))
    randClick(1075,820,10,10,0) #초기화 클릭

    result=client_utils.searchImg('sanga.png',beforeDelay=0, afterDelay=0, _region=(480, 455, 400, 300))
    if(result==0):
      return 0, "상아탑 클릭 실패"
...
def guild(sio, data,btn_name, character_name):
  ...
  randClick(555,745,10,10,0)  #기본기부 받기
  client_utils.searchImg('confirm.png',beforeDelay=1, afterDelay=1, accuracy=0.9, _region=(920,580,300,200))
...
def store(sio, data,btn_name, character_name):
  ...
  #일괄 구매
  result=client_utils.searchImg('allBuy.png', beforeDelay=0, afterDelay=0, _region=(1300,790,350,200))
  if(result==0):
    return 0, "일괄구매 실패"
...
def party_dun_entry(sio, data,btn_name, character_name):
  ...
  keyboard('t') #파티설정
  result=client_utils.searchImg('party_dun_entry.png',beforeDelay=0, afterDelay=0, _region=(960, 755, 200, 100))
  if(result==0):
    randClick(950,795,5,5,0)
...
def item_exchange(sio, data, btn_name, character_name):
  ...
  randClick(1363,338,10,10,0) #장비메뉴

  slot_x=1286
  for i in range(3):
    randClick(slot_x,408,10,10,0) #장비클릭
    result_1=client_utils.searchImg('low_sell_text.png', beforeDelay=1, afterDelay=0, justChk=True, _region=(965,460,300,100), accuracy=0.7)
    ...
    if result_1!=0:
      ...
      numbers = ''.join(re.findall(r'\d+', re.sub(r"[.,]", "", text[0])))  #문자와 , . 제거 후 숫자만 남김
      if numbers=="10":
        randClick(1052,815,10,10,0) #확인 클릭
        randClick(1040,690,10,10,0) #등록 클릭
      else:
        escKey()
        slot_x=slot_x+77
    else:
